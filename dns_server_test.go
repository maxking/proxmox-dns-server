...
func (m *MockProxmoxManager) GetInstanceByIdentifier(identifier string) (ProxmoxInstance, bool) {
	m.mutex.RLock()
...
func (m *MockProxmoxManager) AddInstance(identifier string, instance ProxmoxInstance) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
...
func TestDNSServer_StartWithCancelledContext(t *testing.T) {
...
// TestDNSServer_periodicRefresh tests the periodic refresh functionality
func TestDNSServer_periodicRefresh(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
// TestDNSServer_Integration tests the DNS server with actual network binding
